...
func DevToolsConnections(svc *httpDevToolsConnections) Option {
	return func(srv *Server) {
		if srv.srvHTTP != nil {
			srv.httpDevToolsConnections = svc
			svc.maxBatchSize = srv.maxBatchSize
			svc.maxParallelBatch = srv.maxParallelBatch
			svc.SetRoutes(srv.Fiber())
		}
...
func DevToolsGameConfigs(svc *httpDevToolsGameConfigs) Option {
	return func(srv *Server) {
		if srv.srvHTTP != nil {
			srv.httpDevToolsGameConfigs = svc
			svc.maxBatchSize = srv.maxBatchSize
			svc.maxParallelBatch = srv.maxParallelBatch
			svc.SetRoutes(srv.Fiber())
		}
